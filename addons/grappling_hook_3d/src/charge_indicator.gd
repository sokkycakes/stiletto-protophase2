...
@onready var charge1: ColorRect = $PanelContainer/HBoxContainer/Charge1
@onready var charge2: ColorRect = $PanelContainer/HBoxContainer/Charge2
@onready var charge3: ColorRect = $PanelContainer/HBoxContainer/Charge3

var charges: Array[ColorRect] = []
...
func apply_charge_properties():
	...
	for charge in charges:
		if is_instance_valid(charge):
			...
			# Apply corner radius if the ColorRect supports it
			if charge.has_method("set_corner_radius"):
				charge.set_corner_radius(charge_corner_radius)
	
	# Update spacing in the HBoxContainer
	var hbox = $PanelContainer/HBoxContainer
	if is_instance_valid(hbox):
		hbox.add_theme_constant_override("separation", charge_spacing)
...
func update_charges(current_charges: int):
	...
	for i in range(charges.size()):
		if is_instance_valid(charges[i]):
			charges[i].color = active_color if i < current_charges else inactive_color 

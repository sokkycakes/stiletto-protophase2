...
var map_directories: Array[String] = [
	"res://maps/",      # Primary maps directory
	"res://scenes/"     # Secondary directory for game scenes that can be used as maps
]
...
func sync_lobby_settings(settings: Dictionary) -> void:
	...
	var mode_names = ["Deathmatch", "Team Deathmatch", "Cooperative", "Duel"]
...
func _get_game_mode_name(mode: GameRulesManager.GameMode) -> String:
	match mode:
		...
		GameRulesManager.GameMode.COOPERATIVE:
			return "Cooperative"
		GameRulesManager.GameMode.DUEL:
			return "Duel"
		_:
			return "Unknown"

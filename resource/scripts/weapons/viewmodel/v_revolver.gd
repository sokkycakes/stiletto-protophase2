...
@onready var anim_player : AnimationPlayer = $AnimationPlayer # Replace with the correct path to your AnimationPlayer node
...
func _ready():
	...
	if anim_player == null:
		printerr("Error: AnimationPlayer not found! Check the node path.") #Important to check in case of typos.

	...
	if backstab:
		...
		if target.has_method("take_backstab"):
			...
			if backstab_hit_sound:
				_play_backstab_sound_with_pitch()
			elif impact_sound:
...
func _play_backstab_sound_with_pitch() -> void:
	if backstab_hit_sound == null:
		return
	...
	_backstab_player.stream = backstab_hit_sound
